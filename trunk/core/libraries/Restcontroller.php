...
class Restcontroller extends WaController {

	protected $rest_format = 'restfull'; // Set this in a controller to use a default format
	...
	public function __construct()
	{
		...
		$this->request->format = $this->rest_format; //$this->_detect_format();
	...
	private function _prepare_digest_auth()
	{
		...
		$A1 = md5($digest['username'] . ':' . config_item('rest_realm') . ':' . $valid_pass);
	...
	private function _format_xml($data = array(), $structure = NULL, $basenode = 'dhearts-response')
	{
        ...
        //QuangHuy -- on debug
        private function _format_restfull($data = array(), $structure = NULL, $basenode = 'dhearts-response')
	{
		// turn off compatibility mode as simple xml throws a wobbly if you don't.
		if (ini_get('zend.ze1_compatibility_mode') == 1)
		{
		...
		foreach ($data as $key => $value)
		{
			...
			if (is_numeric($key))
			{
				...
				$key = "entry";
                                ...
                                if (!$myUseAttributeFlag && (is_array($value) || is_object($value))){
                                    foreach ($value as $myLast){
                                        if (!is_array($myLast)) {
                                            $myUseAttributeFlag = true;
                                        }
                                        break;
                                    }
                                } 
			...
			// if there is another array found recrusively call this function
			if ((is_array($value) OR is_object($value))&& !$myUseAttributeFlag)
			{
				$node = $structure->addChild($key);
				// recrusive call.
				$this->_format_restfull($value, $node, $basenode);
			}elseif($myUseAttributeFlag){
                            $myNode = $structure->addChild($key);
                            foreach ($value as $myLastKey => $myLast){
                                $myNode->addAttribute($myLastKey, $myLast);
                            }
                        }
			else
			{
