...
class WaElementRadio extends WaElement
{
	/**
	* Element name
	*
	...
	*/
	...
	function fetchElement($arrElement, $value)
	{
		...
		$class = ( isset($arrElement['class']) ? $arrElement['class'] : 'common_input common_radio' );
		$label_class = ( isset($arrElement['class']) ? 'class="lbl_'.$arrElement['class'].'"' : 'class="lbl_common_input lbl_common_radio"' );
		$wrap_class = ( isset($arrElement['class']) ? 'class="wrap_'.$arrElement['class'].'"' : 'class="wrap_common_input wrap_common_radio"' );

                $mySource = $this->getDataSource($arrElement);
                $myOptions = "";
                if ((is_array($mySource))&&($arrElement['type']=="check_enum")){
                    foreach ($mySource as $k=>$v){
                        $myOptions .= "<label for= '".$id."' class = 'lbl_".$class."' ><input class = '".$class."' type = 'radio' id = '".$id."' name = '".$name."' value = '".$k."' ".(($k==$value)?"checked":"")." />".$v."</label>";
                    }
                }elseif ((is_array($mySource))&&($arrElement['type']=="check_select")){
                    foreach ($mySource as $k=>$v){
                        $myOptions .= "<label for= '".$id."' class = 'lbl_".$class."' ><input class = '".$class."' type = 'radio' id = '".$id."' name = '".$name."' value = '".$v['source_code']."' ".(($v['source_code']==$value)?"checked":"")." />".$v['source_label']."</label>";
                    }
                }
	...
	function fetchViewElement($arrElement, $value)
	{
		...
		$name = $id; //$xmlElement->selectNodes('name',1)->getText();
		$size = ( isset($arrElement['size']) ? 'size="'.$arrElement['size'].'"' : '' );
		$class = ( isset($arrElement['class']) ? 'class="'.$arrElement['class'].' mode_view"' : 'class="common_input common_radio mode_view"' );
		$label_class = ( isset($arrElement['class']) ? 'class="lbl_'.$arrElement['class'].' mode_view"' : 'class="lbl_common_input lbl_common_radio mode_view"' );
		$wrap_class = ( isset($arrElement['class']) ? 'class="wrap_'.$arrElement['class'].'"' : 'class="wrap_common_input wrap_common_radio"' );
